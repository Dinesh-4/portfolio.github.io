...
const skills = skillsContainer.querySelectorAll('.title');

// Set the initial index to 0
let currentSkillIndex = 0;

// Function to activate the next skill
function activateNextSkill() {
  // Deactivate the current skill
  skills[currentSkillIndex].classList.remove('active');
  
  // Move the current skill to the back of the stack
  skills[currentSkillIndex].classList.add('previous');
  
  // Increment the index
  currentSkillIndex++;
  
  // If we've gone past the last skill, start over at the beginning
  if (currentSkillIndex >= skills.length) {
    currentSkillIndex = 0;
  }
  
  // Activate the next skill
  skills[currentSkillIndex].classList.add('active');
  
  // Call this function again after a delay
  setTimeout(activateNextSkill, 2000);
}
...
function sendMail() {
  ...
  emailjs.send("service_xvlth78", "template_0wfppty", params).then(function(res){
    alert("Message Sent !");
